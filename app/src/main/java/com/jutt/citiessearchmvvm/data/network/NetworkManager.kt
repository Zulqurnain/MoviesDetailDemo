...
//                TODO if user is Logged in Revoke Authentication and LOGOUT
    ...
    fun searchCitiesAPI(
        geonamesUserName: String = BuildConfig.GEONAMEORG_USERNAME,
        searchText: String,
        noOfRecords: Int
    ): Call<SearchCitiesResponse> {
        val body = mutableMapOf<String,Any?>(
            "name_startsWith" to searchText,
            "maxRows" to noOfRecords,
            "username" to geonamesUserName,
        )
        return apiService.searchCities(options = body)
    }
